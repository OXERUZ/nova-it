...
export default function Admin(){
 ...
 const [students,setStudents]=useState<Student[]>([]); const [groups,setGroups]=useState<Group[]>([]); const [courses,setCourses]=useState<Course[]>([]); const [leads,setLeads]=useState<Lead[]>([]); const [payments,setPayments]=useState<Payment[]>([]); const [attendance,setAttendance]=useState<Attendance[]>([]); const [staff,setStaff]=useState<Student[]>([]); const [events,setEvents]=useState<any[]>([]); const [query,setQuery]=useState("");
 ...
 async function load(){setLoading(true);setError(""); const [s,g,c,l,p,att,st,e]=await Promise.all([
 ...
 ]); const errs=[s,g,c,l,p,att,st,e].find(x=>x.error); if(errs)setError(errs.error?.message||""); setStudents((s.data||[]).filter((x:any)=>x.role==="student")); setGroups(g.data||[]);setCourses(c.data||[]);setLeads(l.data||[]);setPayments(p.data||[]);setAttendance(att.data||[]);setStaff(st.data||[]);setEvents(e.data||[]);setLoading(false); }
...
function Groups({groups,students,onAdd,onRefresh}:{groups:Group[];students:Student[];onAdd:()=>void;onRefresh:()=>void}){return <><Head eyebrow="GROUP MANAGEMENT" title="Guruhlar." sub="Kurs, daraja, sig‘im va o‘quvchilar tarkibini boshqaring." action={<button className="btn primary" onClick={onAdd}><Plus size={15}/> Guruh yaratish</button>}/><div className="crm-card-grid">{groups.map(g=><div className="crm-panel group-card" key={g.id}><div className="group-top"><span className="status status-new">{g.active?"ACTIVE":"PAUSED"}</span><BookOpen size={17}/></div><h3>{g.name}</h3><p>{g.course} · {g.level}</p><div className="group-stats"><span>Sig‘im <b>{g.capacity}</b></span><span>O‘quvchi <b>{students.filter(s=>false).length}+</b></span></div></div>)}{!groups.length&&<div className="crm-panel">Guruhlar hali yo‘q.</div>}</div></>}
...
function StudentDrawer({student,onClose,onSaved}:{student:Student;groups:Group[];courses:Course[];onClose:()=>void;onSaved:()=>void}){const [f,setF]=useState(student);const [saving,setSaving]=useState(false);async function save(){setSaving(true);const {error}=await supabase.from("profiles").update({full_name:f.full_name,email:f.email,phone:f.phone,birth_date:f.birth_date,city:f.city,address:f.address,started_at:f.started_at,status:f.status,notes:f.notes,avatar_url:f.avatar_url,updated_at:new Date().toISOString()}).eq("id",f.id);setSaving(false);if(error)alert(error.message);else await onSaved()}return <Modal title={f.full_name} onClose={onClose}><div className="profile-hero"><div className="avatar avatar-lg">{f.full_name.split(" ").map(x=>x[0]).slice(0,2).join("")}</div><div><div className="eyebrow">STUDENT PROFILE</div><h3>{f.full_name}</h3><p>{f.role} · {f.status}</p></div></div><div className="drawer-grid"><FormInput label="Ism va familiya" value={f.full_name} onChange={v=>setF({...f,full_name:v})}/><FormInput label="Telefon" value={f.phone||""} onChange={v=>setF({...f,phone:v})}/><FormInput label="Email" value={f.email||""} onChange={v=>setF({...f,email:v})}/><FormInput label="Tug‘ilgan sana" type="date" value={f.birth_date||""} onChange={v=>setF({...f,birth_date:v})}/><FormInput label="Shahar" value={f.city||""} onChange={v=>setF({...f,city:v})}/><FormSelect label="Holat" value={f.status||"active"} options={[["active","Faol"],["frozen","Muzlatilgan"],["completed","Tugatgan"],["removed","Chiqarilgan"]]} onChange={v=>setF({...f,status:v})}/></div><div className="profile-tabs"><span>Davomat</span><span>To‘lovlar</span><span>Baholar</span><span>Topshiriqlar</span><span>Sertifikat</span><span>Activity</span></div><div className="field"><label>Manzil</label><input value={f.address||""} onChange={e=>setF({...f,address:e.target.value})}/></div><div className="field" style={{marginTop:14}}><label>Izohlar</label><textarea value={f.notes||""} onChange={e=>setF({...f,notes:e.target.value})}/></div><button className="btn primary btn-lg" style={{width:"100%",marginTop:18}} onClick={save} disabled={saving}>{saving?"Saqlanmoqda...":"Profilni saqlash"}<Check size={16}/></button></Modal>}
function CreateGroup({courses,onClose,onSaved}:{courses:Course[];onClose:()=>void;onSaved:()=>void}){const [name,setName]=useState("");const [course,setCourse]=useState(courses[0]?.name||"");const [level,setLevel]=useState("Beginner");const [capacity,setCapacity]=useState("20");async function save(e:any){e.preventDefault();const {error}=await supabase.from("groups").insert({name,course,level,capacity:Number(capacity)});if(error)alert(error.message);else await onSaved()}return <Modal title="Guruh yaratish" onClose={onClose}><form onSubmit={save}><FormInput label="Guruh nomi" value={name} onChange={setName} required/><FormSelect label="Kurs" value={course} options={courses.map(c=>[c.name,c.name])} onChange={setCourse}/><FormInput label="Daraja" value={level} onChange={setLevel}/><FormInput label="Sig‘im" type="number" value={capacity} onChange={setCapacity}/><button className="btn primary btn-lg" style={{width:"100%",marginTop:18}}>Guruhni saqlash <Check size={16}/></button></form></Modal>}
function CreateCourse({onClose,onSaved}:{onClose:()=>void;onSaved:()=>void}){const [name,setName]=useState("");const [code,setCode]=useState("");const [duration,setDuration]=useState("6");const [price,setPrice]=useState("0");async function save(e:any){e.preventDefault();const {error}=await supabase.from("courses").insert({name,code:code||null,duration_months:Number(duration),price:Number(price)});if(error)alert(error.message);else await onSaved()}return <Modal title="Kurs qo‘shish" onClose={onClose}><form onSubmit={save}><FormInput label="Kurs nomi" value={name} onChange={setName} required/><FormInput label="Kod" value={code} onChange={setCode}/><FormInput label="Davomiyligi (oy)" type="number" value={duration} onChange={setDuration}/><FormInput label="Narxi" type="number" value={price} onChange={setPrice}/><button className="btn primary btn-lg" style={{width:"100%",marginTop:18}}>Kursni saqlash <Check size={16}/></button></form></Modal>}
function CreatePayment({students,courses,onClose,onSaved}:{students:Student[];courses:Course[];onClose:()=>void;onSaved:()=>void}){const [f,setF]=useState<any>({payment_date:new Date().toISOString().slice(0,10),method:"cash",status:"paid"});async function save(e:any){e.preventDefault();const {error}=await supabase.from("payments").insert({...f,amount:Number(f.amount)});if(error)alert(error.message);else await onSaved()}return <Modal title="To‘lov kiritish" onClose={onClose}><form onSubmit={save}><FormSelect label="O‘quvchi" value={f.student_id||""} options={students.map(s=>[s.id,s.full_name])} onChange={v=>setF({...f,student_id:v})} required/><FormSelect label="Kurs" value={f.course_id||""} options={courses.map(c=>[c.id,c.name])} onChange={v=>setF({...f,course_id:v})}/><FormInput label="Summa" type="number" value={f.amount||""} onChange={v=>setF({...f,amount:v})} required/><FormInput label="Sana" type="date" value={f.payment_date} onChange={v=>setF({...f,payment_date:v})}/><FormSelect label="Usul" value={f.method} options={[["cash","Naqd"],["card","Karta"],["transfer","Transfer"],["online","Online"],["other","Boshqa"]]} onChange={v=>setF({...f,method:v})}/><div className="field"><label>Izoh</label><textarea value={f.note||""} onChange={e=>setF({...f,note:e.target.value})}/></div><button className="btn primary btn-lg" style={{width:"100%",marginTop:18}}>To‘lovni saqlash <Check size={16}/></button></form></Modal>}
function CreateAttendance({students,groups,onClose,onSaved}:{students:Student[];groups:Group[];onClose:()=>void;onSaved:()=>void}){const [f,setF]=useState<any>({attendance_date:new Date().toISOString().slice(0,10),status:"present"});async function save(e:any){e.preventDefault();const {error}=await supabase.from("attendance").upsert(f,{onConflict:"student_id,attendance_date"});if(error)alert(error.message);else await onSaved()}return <Modal title="Davomat kiritish" onClose={onClose}><form onSubmit={save}><FormSelect label="O‘quvchi" value={f.student_id||""} options={students.map(s=>[s.id,s.full_name])} onChange={v=>setF({...f,student_id:v})} required/><FormSelect label="Guruh" value={f.group_id||""} options={groups.map(g=>[g.id,g.name])} onChange={v=>setF({...f,group_id:v})}/><FormInput label="Sana" type="date" value={f.attendance_date} onChange={v=>setF({...f,attendance_date:v})}/><FormSelect label="Holat" value={f.status} options={[["present","Keldi"],["absent","Kelmagan"],["late","Kechikdi"],["excused","Uzrli"]]} onChange={v=>setF({...f,status:v})}/><div className="field"><label>Izoh</label><textarea value={f.note||""} onChange={e=>setF({...f,note:e.target.value})}/></div><button className="btn primary btn-lg" style={{width:"100%",marginTop:18}}>Davomatni saqlash <Check size={16}/></button></form></Modal>}
function FormInput({label,value,onChange,type="text",required=false,placeholder=""}:{label:string;value:string;onChange:(v:string)=>void;type?:string;required?:boolean;placeholder?:string}){return <div className="field" style={{marginTop:12}}><label>{label}</label><input type={type} value={value} onChange={e=>onChange(e.target.value)} required={required} placeholder={placeholder}/></div>};function FormSelect({label,value,options,onChange,required=false}:{label:string;value:string;options:string[][];onChange:(v:string)=>void;required?:boolean}){return <div className="field" style={{marginTop:12}}><label>{label}</label><select value={value} onChange={e=>onChange(e.target.value)} required={required}><option value="">Tanlang...</option>{options.map(([v,l])=><option key={v} value={v}>{l}</option>)}</select></div>}
